...
これらはSassにおいて様々な機能を持っていて、詳しくは以下のようになっています。
...
Sassは、SCSSとSassのファイルをインポートできるようにするためにCSSの `@import` ルールを拡張しています。
インポートしたSCSSとSassの全てのファイルは出力する一つのCSSファイルにマージされます。
...
Sassは、現在のディレクトリ内の他のSassファイルとRack、Rails、Merbの管理下のディレクトリ内のSassファイルを探します。
[`:load_paths`](#load_paths-option)を使って指定したり、コマンドラインで `--load-path` オプションで指定することで探索するディレクトリを追加することもできます。
...
`@import` はインポートするファイルのファイル名を受け取ります。
...
上記のいずれの条件も満たさず、拡張子が `.scss` や `.sass` のSassファイルやSCSSファイルはインポートされます。
拡張子がない場合は、Sassは `.scss` や `.sass` の拡張子のファイルを探して、インポートします。
...
インポートの際に `#{}` というインターポレーションを含んでいる場合、一定の制限があります。
...
インポートしたいがCSSファイルにコンパイルしたくないSCSSファイルやSassファイルがある場合、
ファイル名の最初にアンダースコアを追加することができます。
こうすることで、通常のCSSファイルにそのファイルをコンパイルしないようにSassに指示します。
アンダースコアを使わずにこれらのファイルをインポートすることができます。
...
例えば、 `_colors.scss` は `colors.scss` と一緒に存在しない場合があります。
...
ドキュメントの最上位レベルに `@import` を定義することが一番使いやすい場合が多いですが、CSSルールや `@media` ルール内に @import を含めることができます。
最上位にある `@import` 、これは `@import` されたファイルの内容が含まれています。
ベースレベルの`@ Importのような`、これは `@ importを`されたファイルの内容が含まれています。
...
`@mixin` や `@charset` のようなディレクティブは、ドキュメントの最上位レベルでしか定義することができません。
...
ミックスインや制御ディレクティブ内で `@import` をネストすることはできません。
...
例えば、標準的なエラーと深刻なエラーををデザインすると仮定します。
以下のようにマークアップするとして、
...
残念なことに、これは `.seriousError` と同時に `.error` を使うことを覚えていなければいけません。
...
`@extend` ディレクティブは、一つのセレクタは他のセレクタのスタイルを継承するようにSassに指示することで、これらの問題を避ける事ができます。
例えば、
...
これは、 `.error` に定義している全てのスタイルは、 `.seriousError` にも適用させ、 `.seriousError` に固有のスタイルを追加することを意味しています。
要するに、 `.error` クラスに定義されてる全てのスタイルは `.seriousError` クラスにも定義されています。
...
セレクタをマージする時、 `@extend` は不必要な重複を避けるため、
...
`.special.cool` 、 `a:hover` 、 `a.user[href^="http://"]` のように、一つの要素しか関与していない任意のセレクタを拡張することが可能です。
例えば、
...
ただのクラスと同じように、これは `a:hover` に適用された全てのスタイルは `.hoverlink` にも適用されます。
例えば、
...
上述した `.error.intrusion` のように、 `a:hover` で使われているルールは `.hoverlink` にも適用されます。
例えば、
...
`.foo .bar` や `.foo + .bar` のようなセレクタシーケンスは現在のところ拡張できません。
...
時々セレクタシーケンスは別のシーケンスで定義されている他のセレクタを拡張することがあります。
...
スタイルシートが大きくなりすぎます。
上の簡単なサンプルでは、例えば、10個のセレクタが必要です。
代わりに、Sassは使える可能性があるセレクタだけ出力します。
...
2つのシーケンスがマージされる時に、共通するセレクタがない場合、2つの新しいセレクタが生成されます。
2つ目のシーケンスの前にある最初のシーケンスと、最初のシーケンス
例えば、
...
2つのシーケンスがいくつかのセレクタを共有している場合、これらのセレクタは一緒にマージされ、異なる部分(いずれかがまだ存在する場合)だけ別に
...
時々、一つのクラスにスタイルを書いている時
今まで `@extend` しか
HTMLで直接使わない
これは、必要な時は拡張して、必要ない時は無視するスタイルを提供しているSassのライブラリを書いている時は特にそうです。
...
一般的なクラスを使う際に、スタイルシートが生成される時にたくさんのCSSが生成することになり、HTMLで使われている他のクラスと重複するリスクがある。
...
プレースホルダーセレクタは、 `#` や `.` が `%` に置き換えられていること以外はクラスやIDセレクタのように見えます。
プレースホルダーセレクタはクラスやIDのようにどこでも使うことができ、
...
通常、セレクタを拡張する時、 `@extend` が動作しない場合エラーになります。
例えば、 `a.important {@extend .notice}` と記述した場合、 `.notice` を含むセレクタがない場合エラーになります。
`.notice` を含むセレクタが `h1.notice` だけの場合、
`h1` は `a` とコンフリクトを起こし、新しいセレクタが生成されないのでエラーになります。
...
これをするには、セレクタの後に `!optional` フラグを追加するだけです。
例えば、
...
Sassは、あらゆる場所にスタイルをコピーすることで肥大化した
膨大な量のスタイルシートを作成することなく、中に定義されたセレクタに適用される `@media` ブロックの外側にCSSルールを作ることができません。
これは、 `@media` 内(もしくは他のCSSディレクティブ内)で `@extend` を使う場合、
同じディレクティブブロック内で定義されたセレクタだけ拡張することができます。
...
非推奨のユーザーに警告を出す必要があったり、マイナーなミックスインの間違った使い方を修正する等、ライブラリには便利な機能です。
...
1. `--quiet` コマンドラインオプションか `:quiet` Sassオプションで警告をオフにすることができます。
2. 警告の原因となった場所を確認することができるように、スタイルシートトレースがメッセージと一緒に出力されます。
